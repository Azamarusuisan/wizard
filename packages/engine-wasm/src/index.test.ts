...
test("TS river solve fallback emits CFR-trained rows", () => {
  ...
  assert.ok(result.nodes.some((node) => node.id === "root/raise-sizes"));
  ...
  assert.equal(result.informationSets.find((infoSet) => infoSet.nodeId === "root/raise-sizes")?.strategyRef, "raise-sizes");
  ...
  assert.equal(result.rows[0]!.bestRaiseAmount, 250);
...
test("TS river solve fallback uses board in concrete combo equities", () => {
  ...
  const boardedWithTree = solveRiverSpot(100, 66, 250, "Ah Kd 7c", 0, 0, "NLH", "flop 33; turn 66; river 75");
  assert.equal(boarded.nodes[0]!.street, "flop");
  assert.ok(boarded.nodes.some((node) => node.id === "root/turn-low" && node.street === "turn"));
  assert.ok(boarded.nodes.some((node) => node.id === "root/turn-Qs" && node.street === "turn"));
  assert.ok(boardedWithTree.nodes.some((node) => node.id.startsWith("root/turn-low/bet-") && node.street === "turn" && node.actions.join(",") === "fold,call"));
  assert.ok(boardedWithTree.nodes.some((node) => node.id.startsWith("root/turn-Qs/bet-") && node.street === "turn" && node.actions.join(",") === "fold,call"));
  assert.ok(boardedWithTree.nodes.some((node) => node.id.startsWith("root/turn-low/bet-") && node.id.endsWith("/call")));
  assert.equal(boarded.informationSets.find((infoSet) => infoSet.nodeId === "root/turn-low")?.strategyRef, "root/turn-low");
  assert.equal(boarded.informationSets.find((infoSet) => infoSet.nodeId === "root/turn-Qs")?.strategyRef, "root/turn-Qs");
...
test("TS solve fallback reports PLO Fast BR metrics", () => {
  ...
  const plo4Aces = solveRiverSpot(100, 20, 300, "", 0, 0, "PLO4", "flop 50,200,all-in", "balanced", "AA**:ds@50");
