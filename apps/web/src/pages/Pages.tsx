...
export function SolverStudio() {
  ...
  const nodeRows = shown && selectedNode ? rowsForNode(shown, selectedNode, { game, pot, bet, stack, board, villainRange, rakePct, rakeCap, betTree }) : [];
  ...
  return (
    <div className="split">
      <section className="card grid">
        ...
        {ploSampled ? <p className="muted">PLO Fast uses weighted representative hands, sampled equities, and CFR iterations over the capped representative set; the combo cap limits range representation, and full external-sampling MCCFR for the uncapped game tree is still pending.</p> : null}
      </section>
      ...
      <section className="grid">
        {shown ? <>
          ...
          {shown.metrics.ploOpponentSampleCount !== undefined ? <Metric label="PLO villain set" value={`${shown.metrics.ploOpponentSampleCount.toFixed(0)} reps / ${(100 * (shown.metrics.ploOpponentWeightCoverage ?? 0)).toFixed(0)}%`} /> : null}
...
function rowsForNode(result: SolveResult, node: SolveNode, spot: { game: Game; pot: number; bet: number; stack: number; board: string; villainRange: string; rakePct: number; rakeCap: number; betTree: string }): SolverRow[] {
  ...
  if (node.amount !== undefined && node.pot !== undefined && node.id.endsWith("/fold")) return sourceRows.map((row) => betResponseActionRow(row, "fold", node.pot!, 0, spot.rakePct, spot.rakeCap));
  if (node.amount !== undefined && node.pot !== undefined && node.id.endsWith("/call")) return sourceRows.map((row) => betResponseActionRow(row, "call", node.pot!, node.amount!, spot.rakePct, spot.rakeCap));
  if (node.amount !== undefined && node.pot !== undefined) return sourceRows.map((row) => betResponseRow(row, node.pot!, node.amount!, spot.rakePct, spot.rakeCap));
...
function chanceRow(row: SolverRow, nodeId: string, spot: { game: Game; pot: number; bet: number; stack: number; board: string; villainRange: string; rakePct: number; rakeCap: number; betTree: string }): SolverRow {
  ...
  const rake = Math.min((nextPot + bet) * (spot.rakePct / 100), spot.rakeCap);
  const callEv = (equity * (nextPot + bet - rake) - (1 - equity) * bet) / 100;
...
function chanceBetAmounts(spot: { game: Game; pot: number; bet: number; stack: number; betTree: string }, nodeId: string, pot: number): number[] {
  ...
  const boardLen = nodeId.startsWith("root/turn-") ? 4 : nodeId.startsWith("root/river-") ? 5 : 0;
  if (!boardLen) return [spot.bet];
  const tree = parseBetTree(spot.betTree);
  const sizes = boardLen === 4 ? tree.turn : tree.river;
  const amounts = spot.game === "NLH" ? concreteBets(sizes, pot, spot.stack) : concretePotLimitBets(sizes, pot, spot.bet, spot.stack);
...
function betResponseRow(row: SolverRow, pot: number, amount: number, rakePct: number, rakeCap: number): SolverRow {
  const fold = amount / (pot + amount);
  const call = pot / (pot + amount);
  const rake = Math.min((pot + amount) * (rakePct / 100), rakeCap);
  const callEv = (row.equity * (pot + amount - rake) - (1 - row.equity) * amount) / 100;
...
function betResponseActionRow(row: SolverRow, action: "fold" | "call", pot: number, amount = 0, rakePct = 0, rakeCap = 0): SolverRow {
  const rake = Math.min((pot + amount) * (rakePct / 100), rakeCap);
  const ev = action === "fold" ? pot / 100 : (row.equity * (pot + amount - rake) - (1 - row.equity) * amount) / 100;
  return {
    ...row,
