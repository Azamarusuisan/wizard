...
- Current limitation: the production solve path is still default-combo river solving, not the required full postflop CFR tree. NLH and PLO Fast root rows now consider configured bet-tree raise sizes for the abstract raise EV, but this is not full multi-size tree CFR. NLH flop Balanced and PLO Fast gates currently use compact abstractions/proxies.
