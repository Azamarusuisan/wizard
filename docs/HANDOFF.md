...
- Last verified: `bash scripts/verify.sh` exited 0 after exposing the PLO Fast representative-equity MC sample count in metrics and Solver Studio. Latest bench: `cargo bench -p gto_lab_engine --bench engine_bench` exited 0 with `nlh7_eval` ~11.66 ns/eval.
