...
- Last verified: `bash scripts/verify.sh` exited 0 after adding game-specific PLO range pattern-length validation. Targeted checks also passed: `cargo test -p gto_lab_engine --release native_solve_reports_plo_fast_br_metrics` and `pnpm --filter @gto-lab/engine-wasm test`. Latest bench: `cargo bench -p gto_lab_engine --bench engine_bench` exited 0 with `nlh7_eval` ~11.66 ns/eval.
