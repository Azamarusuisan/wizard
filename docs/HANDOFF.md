...
- Last verified: `bash scripts/verify.sh` exited 0 after adding Rust native bet-response rake coverage. Targeted checks also passed: `cargo test -p gto_lab_engine --release native_solve_subtracts_capped_rake_from_showdown_ev` and `cargo clippy -p gto_lab_engine -- -D warnings`. Latest bench: `cargo bench -p gto_lab_engine --bench engine_bench` exited 0 with `nlh7_eval` ~11.66 ns/eval.
