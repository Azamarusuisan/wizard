...
- Last verified: `bash scripts/verify.sh` exited 0 after zeroing unreachable combos on exact NLH public-card chance branches. Targeted checks also passed: `cargo test -p gto_lab_engine --release native_solve_uses_shared_river_strategy_rows`, `pnpm --filter @gto-lab/engine-wasm test`, and `pnpm --filter @gto-lab/web typecheck`. Latest bench: `cargo bench -p gto_lab_engine --bench engine_bench` exited 0 with `nlh7_eval` ~11.66 ns/eval.
