...
- Last verified: `pnpm --filter @gto-lab/web typecheck` and `pnpm exec playwright test apps/web/tests/core-flows.spec.ts --grep "solver runs"` exited 0 after clarifying board-filtered PLO sample-set display. Latest full verify: `bash scripts/verify.sh` exited 0 after adding conditional raise-size mixes. Latest bench: `cargo bench -p gto_lab_engine --bench engine_bench` exited 0 with `nlh7_eval` ~11.66 ns/eval.
