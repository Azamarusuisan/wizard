...
- Last verified: `bash scripts/verify.sh` exited 0 after adding NLH exact public-card chance branches. Targeted checks also passed: `cargo test -p gto_lab_engine --release native_solve_uses_shared_river_strategy_rows`, `pnpm --filter @gto-lab/engine-wasm test`, `pnpm --filter @gto-lab/web typecheck`, `pnpm --filter @gto-lab/web test`, `cargo clippy -p gto_lab_engine -- -D warnings`, and `pnpm exec playwright test apps/web/tests/core-flows.spec.ts --grep "solver runs"`. Latest bench: `cargo bench -p gto_lab_engine --bench engine_bench` exited 0 with `nlh7_eval` ~11.66 ns/eval.
