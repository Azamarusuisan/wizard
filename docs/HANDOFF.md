...
- The compact flop continuation now uses three explicit chance branches per later street, with deck-count-derived sample mass, instead of one fixed continuation scalar. The flop-to-turn and turn-to-river layers now use sampled runout-card exact equities for each representative bucket; this is still a sampled proxy, not exhaustive turn/river chance.
...
- Last verified: `bash scripts/verify.sh` exited 0 after adding deck-count-derived chance mass to the compact flop continuation. Latest bench: `cargo bench -p gto_lab_engine --bench engine_bench` exited 0 with `nlh7_eval` ~11.66 ns/eval.
